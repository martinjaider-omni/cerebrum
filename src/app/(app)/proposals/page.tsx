...
export default async function ProposalsPage({ searchParams }: { searchParams: { q?: string; status?: string; sort?: string; page?: string } }) {
  ...
  return (
    <div className="p-8">
      ...
      {/* Filter bar */}
      <form method="GET" action="/proposals" className="flex flex-wrap items-end gap-4 mb-6 bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex-1 min-w-[200px]">
          <label htmlFor="q" className="block text-xs font-medium text-gray-500 mb-1">Buscar</label>
          <input id="q" name="q" type="text" defaultValue={q} placeholder="Nombre o cliente..."
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#3E95B0]" />
        </div>
        <div className="min-w-[140px]">
          ...
          <select id="status" name="status" defaultValue={status || 'all'}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3E95B0]">
        ...
        <button type="submit" className="bg-[#3E95B0] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#255664] transition">
          Filtrar
        </button>
      </form>
      ...
      {proposals.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          <p className="text-lg">No hay propuestas</p>
          <p className="text-sm mt-1">{q || status ? 'Intenta ajustar los filtros' : 'Crea tu primera propuesta'}</p>
        </div>
      ) : (
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6">
              ...
              <div className="flex gap-2">
                {page > 1 ? (
                  <Link href={buildHref({ page: String(page - 1) })} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition">Anterior</Link>
                ) : (
                  <span className="px-4 py-2 text-sm border border-gray-200 rounded-lg text-gray-300">Anterior</span>
                )}
                {page < totalPages ? (
                  <Link href={buildHref({ page: String(page + 1) })} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition">Siguiente</Link>
                ) : (
                  <span className="px-4 py-2 text-sm border border-gray-200 rounded-lg text-gray-300">Siguiente</span>
                )}
              </div>
            </div>
          )}
